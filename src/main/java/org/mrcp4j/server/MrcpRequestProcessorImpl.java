...
public class MrcpRequestProcessorImpl implements MrcpRequestProcessor
{
    ...
    private Map<String, MrcpRequestHandler> _requestHandlers = Collections.synchronizedMap(new HashMap<String, MrcpRequestHandler>());

    private Map<String, MrcpSessionImpl> _sessions = Collections.synchronizedMap(new HashMap<String, MrcpSessionImpl>());

    private static String getRequestKey(MrcpRequest request)
    {
        StringBuilder sb = new StringBuilder(request.getHeader(MrcpHeaderName.CHANNEL_IDENTIFIER)
            .getValueString());
        sb.append(':')
    ...
    public MrcpResponse processRequest(MrcpRequest request)
    {
        ...
        try
        {
            ...
            { // channel-identifier header missing
            ...
            else
            {
                ...
                { // no available request handler
                    response = session.createResponse(MrcpResponse.STATUS_RESOURCE_NOT_ALLOCATED, MrcpRequestState.COMPLETE);
                }
                else
                {
                    synchronized(session)
                    {
                        ...
                        if(response == null)
                        {
                            ...
                            response = session.createResponse(MrcpResponse.STATUS_SERVER_INTERNAL_ERROR, MrcpRequestState.COMPLETE);
                        }
                        else
                        {
                            _log.debug("MrcpRequestProcessorImpl got response from request handler.");
                            session._ready = true;
                            if(response.getRequestState()
                                .equals(MrcpRequestState.COMPLETE))
                            {
        ...
        { // invalid channel-identifier header value
    ...
    public MrcpEvent getNextEvent(MrcpRequest request)
    {
        ...
        if(event == null || event.getRequestState()
            .equals(MrcpRequestState.COMPLETE))
        {
    ...
    private static class MrcpSessionImpl implements MrcpSession
    {
         ...
         */
        public synchronized void postEvent(MrcpEvent event) throws TimeoutException, IllegalStateException, NullPointerException
        {
            if(!_ready)
            {
            ...
            try
            {
                ...
                _complete = event.getRequestState()
                    .equals(MrcpRequestState.COMPLETE);
                //                if (_complete) {
